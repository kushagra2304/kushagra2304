<h1 align="center">Hey 👋, I'm Kushagra Sharma</h1>
<h3 align="center">
AI & Full-Stack Developer | MERN • Python • LLMs • RAG
</h3>

<p align="center">
I’m a passionate developer focused on building <b>scalable web applications</b> and 
<b>intelligent AI-powered systems</b>. I enjoy working across the stack — from clean UIs 
to robust backend services — and experimenting with modern AI tools like LLMs, 
LangChain, and vector databases to solve real-world problems.
</p>

---

### 🚀 What I Work With

- 🌐 **Full-Stack Development:** MERN Stack (MongoDB, Express, React, Node.js)
- 🗄️ **Databases:** MongoDB, MySQL (schema design, queries, optimization)
- 🧠 **AI & LLMs:** LangChain, RAG pipelines, embeddings, Hugging Face, OpenAI / Gemini APIs
- 🔍 **Vector Databases:** ChromaDB, Pinecone (semantic search, retrieval systems)
- 🐍 **Python Development:** Flask APIs, Streamlit apps, automation workflows
- ⚙️ **Backend & APIs:** REST APIs, authentication, scalable service design
- 🤖 **Applied AI:** Contract summarization, code analysis, AI agents, automation tools

---

### 🌱 Currently Exploring
- Advanced **RAG architectures**
- **LLM agents & workflows**
- Productionizing AI systems
- Cloud deployment & monitoring

---

### 🤝 Open To
- Collaborating on **AI / LLM / Full-Stack projects**
- Building real-world, production-ready applications
- Learning fast in startup-style environments

---

### 📫 Connect With Me
- 📧 Email: <b>buildwithkushagra@gmail.com</b>
- 💻 GitHub: <a href="https://github.com/kushagra2304">github.com/kushagra2304</a>

---

<h3 align="left">🛠 Languages & Tools</h3>

<div align="center">
  <img src="https://img.shields.io/badge/-JavaScript-333333?style=flat&logo=javascript&logoColor=F7DF1E"/>
  <img src="https://img.shields.io/badge/-Python-333333?style=flat&logo=python&logoColor=3776AB"/>
  <img src="https://img.shields.io/badge/-React-333333?style=flat&logo=react&logoColor=61DAFB"/>
  <img src="https://img.shields.io/badge/-Node.js-333333?style=flat&logo=node.js&logoColor=339933"/>
  <img src="https://img.shields.io/badge/-MongoDB-333333?style=flat&logo=mongodb&logoColor=47A248"/>
  <img src="https://img.shields.io/badge/-MySQL-333333?style=flat&logo=mysql&logoColor=4479A1"/>
  <img src="https://img.shields.io/badge/-Flask-333333?style=flat&logo=flask&logoColor=FFFFFF"/>
  <img src="https://img.shields.io/badge/-Streamlit-333333?style=flat&logo=streamlit&logoColor=FF4B4B"/>
  <img src="https://img.shields.io/badge/-LangChain-333333?style=flat"/>
  <img src="https://img.shields.io/badge/-Git-333333?style=flat&logo=git&logoColor=F05032"/>
</div>
